      ...
      <div style={{ padding: '30px', fontFamily: 'Arial', maxWidth: '800px', margin: '0 auto' }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px', backgroundColor: '#f8f9fa', padding: '20px', borderRadius: '8px', boxShadow: '0 4px 8px rgba(0,0,0,0.1)' }}>
            ...
            <div style={{ display: 'flex', gap: '15px' }}>
              <div style={{ flex: 1 }}>
                ...
                <input type="number" value={weight} onChange={(e) => setWeight(e.target.value)} required placeholder="Misal: 650 Kilo" style={inputStyle} />
                {totalKg > 0 && <small style={{ color: '#000080', fontWeight: 'bold' }}>{displayMun} من {displayKilo} کلو</small>}
              </div>
              <div style={{ flex: 1 }}><label><b>Rate (Per Maund):</b></label><input type="number" value={rate} onChange={(e) => setRate(e.target.value)} required placeholder="Rate likhein..." style={inputStyle} /></div>
            </div>
            ...
            <div style={{ backgroundColor: isBaich ? '#e2e3e5' : '#e9ecef', padding: '15px', borderRadius: '5px', border: `1px solid ${isBaich ? '#d6d8db' : '#ccc'}` }}>
              ...
              <div style={{ display: 'flex', gap: '15px' }}>
                <div style={{ flex: 1 }}><label><b>Commission (%):</b></label><input type="number" step="0.01" value={commPercent} onChange={(e) => setCommPercent(e.target.value)} placeholder="Misal: 2" style={inputStyle} /></div>
                <div style={{ flex: 1 }}><label><b>Labour (%):</b></label><input type="number" step="0.01" value={laborPercent} onChange={(e) => setLaborPercent(e.target.value)} placeholder="Misal: 1.3" style={inputStyle} /></div>
                ...
                <div style={{ flex: 1 }}><label><b>Market Fee (%):</b></label><input type="number" step="0.01" value={marketFeePercent} onChange={(e) => setMarketFeePercent(e.target.value)} placeholder="Misal: 1.5" style={inputStyle} /></div>
              </div>
            </div>
            ...
            <div style={{ backgroundColor: isBaich ? '#d1e7dd' : '#fff3cd', padding: '15px', borderRadius: '5px', border: `1px solid ${isBaich ? '#198754' : '#ffeeba'}`, marginTop: '10px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                <span>Gross Amount:</span> <strong>Rs. {grossAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong>
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '20px', color: '#198754' }}>
                <span><b>NET SAFFA AMOUNT:</b></span> <span><b>Rs. {netAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</b></span>
              </div>
            </div>

            <div style={{ display: 'flex', gap: '15px', marginTop: '10px' }}>
              <button type="submit" onClick={() => { printFlagRef.current = true; }} style={{ flex: 1, padding: '15px', backgroundColor: '#198754', color: 'white', border: 'none', borderRadius: '5px', fontSize: '18px', fontWeight: 'bold', cursor: 'pointer' }}>💾 Save & Print 🖨️</button>
              <button type="submit" onClick={() => { printFlagRef.current = false; }} style={{ flex: 1, padding: '15px', backgroundColor: '#000080', color: 'white', border: 'none', borderRadius: '5px', fontSize: '18px', fontWeight: 'bold', cursor: 'pointer' }}>💾 Sirf Save Karein</button>
            </div>
        ...
        <div className="print-only urdu-text" dir="rtl" style={{ border: '0px', backgroundColor: 'white', color: '#000' }}>
          <div className="d-flex justify-content-between align-items-center border-bottom border-dark pb-2 mb-2">
            <div className="text-right" style={{ width: '40%' }}><h2 className="mb-0 fw-bold" style={{ color: '#000080' }}>{DUKAN_INFO.nameUrdu}</h2><p className="mb-0 fw-bold">{DUKAN_INFO.addressUrdu}</p></div>
            <div className="text-center" style={{ width: '20%' }}><span style={{ fontSize: '30px' }}>🌾</span></div>
            <div className="text-left" dir="ltr" style={{ width: '40%', textAlign: 'left' }}><h4 className="mb-0 fw-bold" style={{ color: '#000080', fontFamily: 'Arial' }}>{DUKAN_INFO.nameEng}</h4><p className="mb-0 fw-bold" style={{ fontFamily: 'Arial' }}>{DUKAN_INFO.addressEng}</p></div>
          </div>
          <div className="d-flex justify-content-between border-bottom border-dark pb-2 mb-4 fs-6">
             ...
             <div dir="ltr" style={{ fontFamily: 'Arial' }}><span className="urdu-text fw-bold me-2">{DUKAN_INFO.phone2Urdu}</span></div>
          </div>
