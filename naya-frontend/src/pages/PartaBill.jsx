...
function PartaBill() {
  ...
  const handleItemChange = (index, field, value) => {
    ...
    if (field === 'weight' || field === 'rate') {
      ...
      updatedItems[index].amount = (w / 40) * r;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    for (let i = 0; i < items.length; i++) {
      if (!items[i].cropType || !items[i].weight || !items[i].rate) {
        setStatus(`❌ Item ${i + 1} mein tafseel zaroori hai!`);
        return;
      }
    }
                ...
                return (
                  <div key={index} style={{ display: 'flex', gap: '10px', marginBottom: '10px', backgroundColor: 'white', padding: '10px', borderRadius: '5px', alignItems: 'flex-start' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <input type="number" value={item.weight} onChange={(e) => handleItemChange(index, 'weight', e.target.value)} required style={inputStyle} />
                      {iKg > 0 && <small style={{ display: 'block', color: '#000080', fontWeight: 'bold', marginTop: '2px' }}>{iMun} من {iKilo} کلو</small>}
                    </div>
