...
// 4. PARCHI SAVE (✅ FIX: Rokar logic removed, only handles Ledgers)
router.post('/add', fetchUser, async (req, res) => {
  try {
    ...
    const newParcha = new Parcha({
      parchaNo: finalParchaNo, transactionType: transactionType, khataCategory: khataCategory,
      partyId: party._id, partyName: party.name, cropType: cropType || 'N/A',
      weight: Number(weight) || 0, rate: Number(rate) || 0,
    ...
    // Main Party ki Transaction
    const newTransaction = new Transaction({
        voucherNo: finalParchaNo, date: Date.now(), transactionType: transactionType,
        khataCategory: khataCategory, partyId: party._id, partyName: party.name,
    ...
    if (Number(mazdoori) > 0) {
      ...
      await new Transaction({
          voucherNo: finalParchaNo, date: Date.now(), transactionType: 'Mazdoori',
          khataCategory: palledar.partyType, partyId: palledar._id, partyName: palledar.name,
          debit: 0, credit: Number(mazdoori), 
          details: `Mazdoori Parchi No: ${finalParchaNo}`
      }).save();
    }
...
// 6. PARCHI DELETE (✅ FIX: Rokar logic removed, only handles Ledgers)
...
router.get('/parties/all', fetchUser, async (req, res) => {
  try {
    const parties = await Party.find().select('name partyType currentBalance balanceType createdAt khataIndex').sort({ name: 1 });
